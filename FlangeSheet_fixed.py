...
class FlangePiece:
    ...
    def read_flangePiece_data(self):
        # 使用正确的资源路径
        path = get_resource_path("Data") + "\\" + str(self.std) + "_" + str(self.tp) + ".csv"
        
        df = pd.read_csv(path, dtype={'DN': "str", 'PN': "str"})
        # print(path)
        # print(df)

        # 选出对应的法兰数据记录行，赋值给sizex
        # sizex = df.loc[(df['DN '] == self.dn) & (df['PN '] == self.pn)]  # 选取满足条件的行，返回dataframe
        self.sizex = df.loc[(df['DN'] == self.dn) & (df['PN'] == self.pn)]  # 选取满足条件的行，返回dataframe
        # print(self.sizex)
...
# 剩余法兰尺寸设置及密封面尺寸设置
    ...
    def buid_t(self):
        df = None
        if self.cB_t_std.currentText() == 'HGJ514<无缝管><B>_供参考':
            path = get_resource_path("Data") + "\\HGJ514无缝管.csv"
            ...
            try:
                ...
                self.lbt_di.setText('(' + str(df2.iloc[0].at['D_i']) + ')')
                self.lbt_s.setText('(' + str(df2.iloc[0].at['S']) + ')')
                self.lbt_sch.setText('(' + str(df2.iloc[0].at['Sch']) + ')')
            except Exception as result:
                self.lbt_d.setText('-')
                self.lbt_di.setText('-')
                self.lbt_s.setText('-')
                self.lbt_sch.setText('-')

        elif self.cB_t_std.currentText() == 'HGJ528Ⅱ系列<有缝管><B>_供参考':
            path = get_resource_path("Data") + "\\HGJ528_2有缝管.csv"
            df = pd.read_csv(path, dtype={'DN': "str", 'DN2': "str"})
            try:
                if self.cB_std.currentText() == 'HGT20592<PN系列>':
                    df2 = df[(df["DN"] == self.cB_dn.currentText())]
            ...
            path = get_resource_path("Data") + "\\GB12459_GBT13401.csv"
            df = pd.read_csv(path, dtype={'DN ': "str"})
            try:
                df2 = df[(df["DN "] == self.cB_dn.currentText()) &
                         (df["Sch"] == self.cB_t_sch.currentText())]
            ...
            path = get_resource_path("Data") + "\\ANSIB36.10M_B36.19M.csv"
            df = pd.read_csv(path, dtype={'NPS': "str"})
            try:
                df2 = df[(df["NPS"] == self.cB_dn.currentText()) &
                         (df["Schedule"] == self.cB_t_sch.currentText())]

                self.lbt_d.setText('(' + str(df2.iloc[0].at['OD']) + ')')
                self.lbt_di.setText('(' + str(df2.iloc[0].at['ID']) + ')')
                self.lbt_s.setText('(' + str(df2.iloc[0].at['t']) + ')')
                self.lbt_sch.setText('(' + str(df2.iloc[0].at['Schedule']) + ')')
            except Exception as result:
                self.lbt_d.setText('-')
                self.lbt_di.setText('-')
                self.lbt_s.setText('-')
            ...
            path = get_resource_path("Data") + "\\HG20553(Ia)系列.csv"
            df = pd.read_csv(path, dtype={'DN ': "str"})
            try:
                df2 = df[(df["DN "] == self.cB_dn.currentText()) &
                         (df["Sch"] == self.cB_t_sch.currentText())]
    ...
    def choose_t(self):
        if self.cB_t_std.currentText() == 'HGJ514<无缝管><B>_供参考':
            ...
            try:
                ...
                if self.cB_t_std.currentText() == 'ANSI B36.10M、B36.19M<A>':
                    path = get_resource_path("Data") + "\\ANSIB36.10M_B36.19M.csv"
                    df = pd.read_csv(path, dtype={'NPS': "str"})
                # HG20533(Ia)系列
                elif self.cB_t_std.currentText() == 'HG20533(Ia)系列':
